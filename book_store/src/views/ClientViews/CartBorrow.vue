<style>
  .cart{
    text-decoration: none; color: inherit;margin-left:90%
  }
...
<template>
 <v-row>
    ...
    class="cart"
    >
         <v-btn color="#ff8000">Quay lại phần mua sách</v-btn>
    </router-link><br/><br/>
   <v-simple-table>
  ...
  <v-row>
      <v-col cols="5">
            <h2 style="margin:10px"> Tổng hóa đơn : ${{totalDate* parseInt(totalPr)}}</h2>
            <h2 style="margin:10px"> Tổng số đơn : {{totalOrder}}</h2>
      </v-col>
       <v-col cols="5"> 
           <v-btn color="red" style="margin-left:80%" @click="showTransport()">Thanh toán</v-btn>
      </v-col>
          <v-dialog 
                    ...
                    <v-stepper-items>
                      ...
                      <v-stepper-content step="2">
                        ...
                        >
                            ...
                            <h3 style="margin:10px">Số tiền thanh toán : ${{totalDate* parseInt(totalPr)}} </h3>
...
import axios from 'axios' 
  export default {
    ...
    methods: {
      ...
      async RegisterTransport(){
          if(this.fullname == '' || this.email == '' || this.address == '' || this.phonenumber == ''){
            ...
            axios.post(`http://localhost:5000/api/transportBorrow/${this.fullname}/${this.email}/${this.address}/${this.phonenumber}/${this.startdate}/${this.enddate}`)
            .then(success=>{
              console.log(success)
            })
            .catch(err=>{
            ...
            this.registerBase = true
             this.registerBase = true
             var Startyear =  parseInt(this.startdate.slice(0,4))
             var Starday =  parseInt(this.startdate.slice(8,10))
             var Startmonth =  parseInt(this.startdate.slice(5,7))
             var Endyear =  parseInt(this.enddate.slice(0,4))
             var Endday =  parseInt(this.enddate.slice(8,10))
             var Endmonth =  parseInt(this.enddate.slice(5,7))
             this.totalDate = ((Endyear-Startyear)*365) + ((Endmonth-Startmonth)*30) + (Endday-Starday)
            // console.log("Start :"+ Startmonth + '-' + Starday + '-' + Startyear)
            // console.log("Start :"+ Endmonth + '-' + Endday + '-' + Endyear)
            console.log(this.totalDate)
          }

      },
      async DeleteAllCart(){
          axios.delete(`http://localhost:5000/api/cartborrow`)
          .then(success=>{
            console.log(success)
          })
          .catch(err=>{
      ...
      async LoadItens(){
        ...
        const API_URL = "http://localhost:5000/api/cartborrow";
        // If the fetch fails,
        try {
    ...
    editedItem(item){
      ...
      axios.put(`http://localhost:5000/api/cartborrow/${item._id}/${item.totalquantity}/${item.totalprice}/${item.booksId}/${item.bookName}/${item.bookImg}`)
      .then(success=>{
        console.log(success)
      })
      .catch(err=>{
    ...
    deleteItem(item) {
        ...
        if(this.dialogDelete == true){
          axios.delete(`http://localhost:5000/api/cartborrow/${item._id}`)
        }
      },
      billClick(){
        if(parseInt(this.totalOrder) < 1){
          ...
          axios.post(`http://localhost:5000/api/bill/${this.totalOrder}/${this.totalDate* parseInt(this.totalPr)}`)
          .then(data=>{
              this.bill = data
          })
          this.end = true
