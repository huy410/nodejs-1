...
$(function () {
    ...
    (function () {
        let username = prompt("Nhập tên tham gia nhóm chat", "tên");
        ...
        $("#logged_as_info").html(`<h2>Tên đang nhập: <strong>` + username + `</strong></h2>`);
    })();
    ...
    socket.on('userConnected', (usernameThatJustConnected) => { //() => {} is a nameless function    
        ...
        const join_log = `<li id=` + notification_id + `><h3>❣️ <strong>` + usernameThatJustConnected + `</strong> đã kết nối nhóm  chat!</h3></li>`;
    ...
    socket.on('userDisconnected', (usernameThatJustDisonnected) => { //() => {} is a nameless function    
        ...
        const left_log = `<li id=` + notification_id + `><h3>🙃 <strong>` + usernameThatJustDisonnected + `</strong> đã ròi nhóm chat!</h3></li>`;
    ...
    socket.on('updateNumUsersOnline', (num_users_online) => {
        $('#num_users_online_number').html('<h3>' + num_users_online + ' người Online</h3>');
    });
